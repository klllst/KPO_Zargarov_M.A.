        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                @if(\Illuminate\Support\Facades\Auth::user()->userable_type == \App\Models\Student::class)
                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        <x-nav-link :href="route('tests.self-tests')" :active="request()->routeIs('tests.index')">
                            {{ __('Учебный план') }}
                        </x-nav-link>
                    </div>

                    <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                        <x-nav-link :href="route('scores.self-scores')" :active="request()->routeIs('tests.index')">
                            {{ __('Мои оценки') }}
                        </x-nav-link>
                    </div>
                @endif
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-transparent text-sm leading-4 font-medium rounded-md text-gray-500 bg-white hover:text-gray-700 focus:outline-none transition ease-in-out duration-150">
                            <div>{{ Auth::user()->userable?->name ??  Auth::user()->userable?->first_name}}</div>
